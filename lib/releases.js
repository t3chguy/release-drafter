...
const findReleases = async ({
  ...
  let lastStableRelease
  lastStableRelease = previousVersion
    ? sortedSelectedReleases.findLast((r) => r.tag_name === previousVersion)
    : sortedSelectedReleases.findLast((r) => !r.prerelease)
  ...
  if (lastStableRelease && lastRelease !== lastStableRelease) {
    log({
      context,
      message: `Last stable release: ${lastStableRelease.tag_name}`,
    })
  }

  return { draftRelease, lastRelease, lastStableRelease }
}
...
const generateReleaseInfo = ({
  ...
  const versionKeyIncrement =
    _versionKeyIncrement ||
    resolveVersionKeyIncrement(mergedPullRequests, config, isPreRelease)
